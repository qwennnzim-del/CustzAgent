...
const Header: React.FC<HeaderProps> = ({ model, onModelChange, onToggleSidebar, onNewChat }) => {
  ...
  const modelOptions: { id: ModelType; name: string; description: string }[] = [
    { id: 'gemini-2.5-flash', name: 'Flash 2.5', description: 'Model standar tercepat & seimbang.' },
    { id: 'gemini-flash-lite-latest', name: 'Lite 2.5', description: 'Super ringan & hemat biaya.' },
    { id: 'gemini-1.5-flash', name: 'Flash 1.5 (Legacy)', description: 'Versi klasik yang stabil.' },
    { id: 'gemini-3-pro-preview', name: 'Pro 3.0', description: 'Reasoning canggih, Coding & Math.' },
    { id: 'imagen-4.0-generate-001', name: 'Imagen (Ultra)', description: 'Buat gambar kualitas pro 8K.' },
    { id: 'gemini-2.5-flash-image', name: 'Edit (Precision)', description: 'Edit gambar dengan akurasi wajah tinggi.' },
  ];
    ...
    <header className="fixed top-0 left-0 right-0 bg-[#0D0D0D] bg-opacity-80 backdrop-blur-sm z-20 border-b border-gray-800/50">
      <nav className="mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between h-16">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <div className="absolute top-full mt-2 w-72 bg-[#1A1A1A] border border-gray-700/50 rounded-lg shadow-lg z-30 right-0 origin-top-right">
